...
function resolvePoolConfig() {
  const db = config.monroeDatabase ?? {};

  if (!db.host || !db.user || !db.name) {
    const error = new Error(
      'Debe configurar DB_HOST_MONROE, DB_USER_MONROE y DB_NAME_MONROE para almacenar los comprobantes de Monroe'
    );
    error.status = 500;
    throw error;
  }
...
export async function getMonroePool() {
  if (!poolPromise) {
    poolPromise = (async () => {
